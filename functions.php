<?php
function ano_6310_link_css_js($hook)
{
   if(
      $hook != 'toplevel_page_ano-6310-draw-attention' && 
      $hook != 'draw-attention_page_ano-6310-draw-attention-import-export' && 
      $hook != 'draw-attention_page_ano-6310-draw-attention-license' && 
      $hook != 'draw-attention_page_ano-6310-draw-attention-use' && 
      $hook != 'draw-attention_page_ano-6310-wpmart-plugins' &&
      $hook != 'draw-attention_page_ano-6310-draw-attention-import-export' &&
      $hook != 'draw-attention_page_ano-6310-draw-attention-use' &&
      $hook != 'draw-attention_page_ano-6310-draw-attention-license' &&
      $hook !='draw-attention_page_ano-6310-wpmart-plugins'
   ...
   wp_enqueue_style('ano-6310-color-style', plugins_url('assets/css/jquery.minicolors.css', __FILE__));
   wp_enqueue_style('ano-6310-font-select-style', plugins_url('assets/css/fontselect.css', __FILE__));
   wp_enqueue_style('ano-6310-font-awesome-5-0-13', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.9.0/css/all.min.css');
   wp_enqueue_style('ano-6310-codemirror-style', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.css');
   wp_enqueue_style('ano-6310-color-style', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.4/jquery.minicolors.min.css');
   wp_enqueue_style('ano-6310-jquery-ui-css', "https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.css");

   
   wp_enqueue_script('ano-6310-font-select-js', plugins_url('assets/js/fontselect.js', __FILE__), array('jquery'));
   wp_enqueue_script('ano-6310-jquery-ui-js', 'https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js', array('jquery'));
   wp_enqueue_script('ano-6310-color-js', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.4/jquery.minicolors.min.js', array('jquery'));
   wp_enqueue_script('ano-6310-codemirror-js', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.js', array('jquery'));
   wp_enqueue_script('ano-6310-common', plugins_url('assets/js/ano-6310-common.js', __FILE__), array('jquery'));
   wp_enqueue_script('ano-6310-json-js', plugins_url('assets/js/json-data.js', __FILE__), array('jquery'));
   wp_enqueue_script('ano-6310-admin-js', plugins_url('assets/js/ano-6310-admin-script.js', __FILE__), array('jquery'));
   wp_enqueue_script('ano-6310-modal-js', plugins_url('assets/js/ano-6310-admin-modal.js', __FILE__), array('jquery'));
   wp_enqueue_script('ano-6310-canvas-js', plugins_url('assets/js/jquery.canvas.js', __FILE__), array('jquery'));
}

function ano_6310_replace($data) {
	if($data == '') return ''; 
...
function ano_6310_get_user_roles(){
...
function ano_6310_extract_data($data)
{
   $array_key = "";
   $array_value = "";
   $i = 0;
...
function ano_6310_search_template($ids, $cssData, $output = 0){
   if(!isset($cssData['search_activation']) && $output) return;
	?>
	<div class="ano-6310-search ano-6310-search-<?php echo esc_attr($ids) ?>">
		<div class="ano-6310-search-container ano-6310-search-template-<?php echo esc_attr($ids) ?>">
			<input type="text" name="ano-6310-search-box"  data-ano-6310-template-id='<?php echo esc_attr($ids) ?>' class="ano-6310-search-box" autocomplete="off" placeholder="<?php echo (isset($cssData['search_placeholder']) && $cssData['search_placeholder'] !== '') ? esc_attr($cssData['search_placeholder']) : '' ?>">
   ...
   function ano_6310_annotation_install()
   {
      global $wpdb;
   ...
   function ano_6310_get_option($name) {
		global $wpdb;
   ...
   function ano_6310_version_status() {
      global $wpdb;
      $db_version = ano_6310_get_option('ano_6310_version_info');
      if(!$db_version){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='ano_6310_version_info'");
            $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('ano_6310_version_info', '".ano_6310_PLUGIN_CURRENT_VERSION."')");
      }
      else{
         $key = ano_6310_get_option('ano_6310_license_key');
         if($db_version != ano_6310_PLUGIN_CURRENT_VERSION && $key){
            ano_6310_check_license($key, true);
            $wpdb->query("UPDATE {$wpdb->prefix}options set 
                        option_value='". ano_6310_PLUGIN_CURRENT_VERSION ."' 
                        where option_name = 'ano_6310_version_info'");
         }	
      }
   } 
   
   function ano_6310_check_license($key, $autoUpdate = false)
   {
      global $wpdb;
   
      $db_key = ano_6310_get_option('ano_6310_license_key');
      if(!$db_key){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='ano_6310_license_key'");
         $wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}options SET option_name = %s, option_value = %s", 'ano_6310_license_key', "{$key}"));
         if(!$wpdb->insert_id) {
            $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}options SET option_value = %s where option_name = %s", "{$key}", 'ano_6310_license_key'));
         }
      }else if($db_key != $key){
         $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}options SET option_value = %s where option_name = %s", "{$key}", 'ano_6310_license_key'));
      }
      ...
      if(!class_exists('ZipArchive')){
         $api_params = array(
            ...
            'license' => $key,
            'item_name' => urlencode('Draw Attention'),
            'url' => home_url(),
            'type' => 'ano'
         );
         ...
         if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (false === $license_data->success) {
               switch ($license_data->error) {
                  case 'invalid_key':
                     $message = __('<p class="ano-6310-error-message">Your have enter invalid license key.</p>');
                     break;
                  case 'site_inactive':
                     $message = __('<p class="ano-6310-error-message">Your license is not active for this URL.</p>');
                     break;
                  default:
                     $message = __('<p class="ano-6310-error-message">An error occurred, please try again.</p>');
                     break;
               }
               return;
            }
         ...
         if (is_wp_error($unzipfile)) {
            echo '<p class="ano-6310-error-message">There was an error unzipping the file.</p>';
            return;
         } else {
            
            if(!$autoUpdate){
               echo "<p class='ano-6310-success-message'>Congratulations! Your license activated successfully.</p>";		
   ...
   function ano_6310_fa_icon_list($startTag, $endTag) {
      $iconArray = array (
   ...
   function ano_6310_extract_item($ids)
   {
      global $wpdb;
      $item_table = $wpdb->prefix . 'ano_6310_item';
      $results = array();
      if($ids) {
         $ids = explode(',', $ids);
         if($ids) {
...
function ano_6310_display_icon($ids, $type, $icons, $hover_icons, $image, $hover_image) {
   echo "
      <div class='ano-6310-template-".esc_attr($ids)."-icon-wrapper'>
         <div class='ano-6310-template-".esc_attr($ids)."-icon'>";
         if($type == 1) {
            $hover_icons = $hover_icons ? $hover_icons : $icons;
            echo "
                  <i class='".esc_attr($icons)." ano-6310-icon'></i>
                  <i class='".esc_attr($hover_icons)." ano-6310-hover-icon'></i>
               ";
         } else{
            $hover_image = $hover_image ? $hover_image : $image;
            echo "
                  <img src='".esc_attr($image)."' class='ano-6310-image'>
                  <img src='".esc_attr($hover_image)."' class='ano-6310-hover-image'>
               ";
         }
   echo  "</div>
      </div>
...
function ano_6310_display_read_more($ids, $target, $text, $url, $output = 1) {
   if(!strlen($text)) return;
   if(!$output) return;
   $target = $target ? "target='_blank'" : '';
   echo "
   <div class='ano-6310-template-".esc_attr($ids)."-read-more'>
      <a href='{$url}' {$target}>{$text}</a>
   </div>
   ";
}

function ano_6310_add_new_media($id, $results = [])
   {
      global $wpdb;
      wp_enqueue_media();
      $style_table = $wpdb->prefix . 'ano_6310_style';
      $item_table = $wpdb->prefix . 'ano_6310_item';
?>
      <div class="ano_6310_add_media">
         <h6>Shortcode</h6>
         <div class="ano_6310_add_media_body_shortcode">
            <input type="text" onclick="this.setSelectionRange(0, this.value.length)" value='[ano_6310_annotation id="<?php echo esc_attr($id); ?>"]' />
         </div>
      </div>
      <br />
      <div class="ano_6310_add_media">
         <h6>Customize Items</h6>
         <div class="ano_6310_add_media_body" id="ano-6310-add-or-edit-items">
            <i class="fas fa-plus-circle ano_6310_add_media_add_new_icon"></i>
            Add/Edit Items
         </div>
      </div>
      <br />
      <div class="ano_6310_add_media">
         ...
         <div class="ano_6310_add_media_body" id="ano-6310-rearrange-items">
            <i class="fas fa-user-cog ano_6310_add_media_add_new_icon"></i>
         </div>
      </div>
      <br />
      <div class="ano_6310_add_media">
         <h6>How to Use</h6>
         <div class="ano_6310_add_media_body">
...
<?php
   ...
   function ano_6310_description($ids, $description, $output = 1){
      if(!$description) return;
      if(!$output) return;
      echo "<div class='ano-6310-template-".esc_attr($ids)."-description'>".esc_attr($description)."</div>";
   }

   function ano_6310_export_full_map_annotation_plugin() {
      ...
      fwrite($fp, pack("CCC",0xef,0xbb,0xbf)); 
		fwrite($fp, $sqlData);				
		fclose($fp);
		echo '<a href="'.$path['url'].'/'.$txtName.'" target="_blank" id="export-draw-attention-plugin">Download</a>';
   }
   
   function ano_6310_import_full_plugin($url) {
		if( !ini_get('allow_url_fopen') ) {
			echo "<p style='color: green; font-size: 14px;'>
			In your cPanel, default allow_url_fopen is not enable. You need to enable. Please contact with hosting providers or 	follow the below URL. <br />
			<a href='https://www.youtube.com/watch?v=tUW6CkZEW8k'>How to Enable Allow_url_fopen in cPanel</a>
   ...
   function ano_6310_validate_url($url){
      if ($url != '' && substr($url, 0, 7) != "http://" && substr($url, 0, 8) != "https://"){
         $url = "http://{$url}";
      }
      return $url;
   }
   function ano_6310_load_templates($js, $counter, $ids) {
      $html = "";
      $customCSS = "
      .ano-6310-annotation-box-{$ids} .ano-6310-point-{$counter}  .ano-6310-pin-main-img {           
         color: red;
         font-size: 15px;
      }    
      .ano-6310-annotation-box-{$ids} .ano-6310-point-{$counter} .ano-6310-pin-hover-img {           
         color: yellow;
         font-size: 15px;
      }   
      ";    
      if($js->viewMoodType == '2'){
         ...
         window.addEventListener('load', function(){
            jQuery('body').append(\"<div class='ano-6310-hover-content ano-6310-hover-content-{$ids}-{$counter}'><div class='ano-6310-hover-content-{$ids}-{$counter}-element'><div class='ano-6310-template-modal-hover-content'>".esc_attr($js->selectModelContent)."</div></div></div>\");
         });
      "; 
      $customCSS .="
      .ano-6310-hover-content-{$ids}-{$counter}-element .ano-6310-template-modal-hover-content{
         font-size:".esc_attr($js->ModalContentFontSize)."px;
         color:".esc_attr($js->ModalContentColor).";
         background: ".esc_attr($js->ModalContentBgColor).";
         padding:2px 10px;
      ...
      $modal_start = '';
      $modal_end = '';   
      if($js->viewMoodType == '1'){
         $className1 = "ano-6310-hover-content";   
         $className2 = "ano-6310-hover-content-{$ids}-{$counter}";
      } else {
         $className1 = "ano-6310-modal-content";   
         $className2 = "ano-6310-modal-content-{$ids}-{$counter}";   
         $modal_start = "<div class='ano-6310-modal ano-6310-popup-{$ids}-{$counter}'><div class='ano-6310-modal-content ano-6310-modal-xl'>";
         $modal_end = "</div></div>";
      }  
      ...
      if($js->elementType == '1' || $js->elementType == '2'){        
         ...
         if($js->selectedTemplate == '01') {
               ...
               window.addEventListener('load', function(){
                  jQuery('body').append(`
                     ...
                     <div class='{$className1} {$className2}'>
                        <div class='{$className2}-element'>
                           <div class='ano-6310-template-01-hover-content'>".esc_attr($js->linkText)."</div>       
                        </div>                    
                     </div>
                     {$modal_end}
                  `);
            ...
            ";        
            $customCSS .= "
            .{$className2}-element .ano-6310-template-01-hover-content{
                  ...
                  <div class='{$className1} {$className2}'>
                     <div class='{$className2}-element'>
                        <div class='ano-6310-tooltip ano-6310-template-02'>      
                           ...
                           ".ano_6310_create_embedded_code($js->tem02EmbeddedLink)."                                     
                           </div>
                           </div>
                        </div>
                     </div>
         ...
         ";  
                                 ...
                                 <div class='ano-6310-template-03-hover-content'>
                                    <div class='ano-6310-template-03-tooltip-testimonial'>
                                       ...
                                       <div class='ano-6310-template-03-tooltip-testimonial-content'>
                                          <div class='ano-6310-template-03-tooltip-testimonial-title'>".esc_attr($js->linkText)."</div>
            ...
            ";         
            $customCSS .= "
               .{$className2}-element .ano-6310-template-03-tooltip-testimonial{
                  background-color: ".esc_attr($js->tempCommonBgColor).";
               }
               .{$className2}-element .ano-6310-template-03-tooltip-testimonial .ano-6310-template-03-tooltip-testimonial-title{
                  font-size: ".esc_attr($js->tempCommonFontSize)."px;
                  line-height: ".esc_attr($js->tempCommonFontSize + 6)."px;
                  color: ".esc_attr($js->tempCommonFontColor).";
                  padding-bottom:5px;
               }
               .{$className2}-element .ano-6310-template-03-tooltip-testimonial .ano-6310-template-03-tooltip-description{
                  color: ".esc_attr($js->openDesFontColor).";
                  font-size:".esc_attr($js->openDesFontSize)."px;
                  line-height:".esc_attr($js->openDesFontSize + 6)."px;
               }
                        ...
                        <div class='{$className2}-element'>
                           <div class='ano-6310-template-04-tooltip-testimonial'>
                              <div class='ano-6310-template-04-tooltip-testimonial-content'>
                                 ...
                                 <div class='ano-6310-template-04-tooltip-title'>".ano_6310_replace(esc_attr($js->linkText))."</div>
                                 </div>
                                 <div class='ano-6310-template-04-tooltip-description'>
                                 ".ano_6310_replace(esc_attr($js->openDescription))."
                                 </div>        
                           </div>
                        </div>
                     </div>
            ...
            $customCSS .= "
               .{$className2}-element .ano-6310-template-04-tooltip-testimonial{
                  background: ".esc_attr($js->tempCommonBgColor).";     
                  text-decoration: none !important;   
               }  
               .{$className2}-element .ano-6310-template-04-tooltip-title{        
                  color: ".esc_attr($js->tempCommonFontColor).";
                  font-size:".esc_attr($js->tempCommonFontSize)."px;
                  line-height:".esc_attr($js->tempCommonFontSize + 6)."px;
                  text-decoration: none !important; ;
               }  
               .{$className2}-element .ano-6310-template-04-tooltip-description{        
                  color: ".esc_attr($js->openDesFontColor).";
                  font-size:".esc_attr($js->openDesFontSize)."px;
                  line-height:".esc_attr($js->openDesFontSize + 6)."px;
                  text-decoration:none !important;
                           ...
                           <div class='ano-6310-template-05-hover-content'>
                              <div class='ano-6310-template-05-tooltip-testimonial'>
                                 <div class='ano-6310-template-05-tooltip-testimonial-content'>
                                    ...
                                    <div class='ano-6310-template-05-tooltip-title'>".ano_6310_replace(esc_attr($js->linkText))."</div>
                                 </div>
            ...
            "; 
            $customCSS .= "
            .{$className2}-element .ano-6310-template-05-hover-content{
               background: ".esc_attr($js->tempCommonBgColor).";        
            }  
            .{$className2}-element .ano-6310-template-05-tooltip-testimonial-content .ano-6310-template-05-tooltip-title {        
               color: ".esc_attr($js->tempCommonFontColor).";
               font-size:".esc_attr($js->tempCommonFontSize)."px;
               line-height:".esc_attr($js->tempCommonFontSize + 6)."px;
            }    

            .{$className2}-element .ano-6310-template-05-tooltip-description {
               color: ".esc_attr($js->openDesFontColor).";
               font-size:".esc_attr($js->openDesFontSize)."px;
               line-height:".esc_attr($js->openDesFontSize + 6)."px;
            }     
   ...
   if($modal_start){
      $customCSS .= ".ano-6310-popup-{$ids}-{$counter}{display: none;}";
   }
  

   wp_register_style("ano-6310-custom-" . esc_attr($ids) . "-" . esc_attr($counter) . "-css", "");
   wp_enqueue_style("ano-6310-custom-" . esc_attr($ids) . "-" . esc_attr($counter) . "-css");
   wp_add_inline_style("ano-6310-custom-" . esc_attr($ids) . "-" . esc_attr($counter) . "-css", $customCSS);

   wp_register_script("ano-6310-template-" . esc_attr($counter) . "-js", "");
   wp_enqueue_script("ano-6310-template-" . esc_attr($counter) . "-js");
   wp_add_inline_script("ano-6310-template-" . esc_attr($counter) . "-js", $html); 
   }
