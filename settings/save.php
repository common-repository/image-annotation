...
if (!empty($_POST['update_style_change']) && $_POST['update_style_change'] == 'Save' && $_POST['id'] != '') {
  ...
  if (!wp_verify_nonce($nonce, 'ano_6310_nonce_field_form')) {
    die('You do not have sufficient permissions to access this pagess.');
  } else {
    $id = sanitize_text_field($_POST['id']);
    $name = sanitize_text_field($_POST['name']);
    $css = ano_6310_extract_data($_POST);
    if ($id) {
      ...
      $url = admin_url("admin.php?page=ano-6310-draw-attention&action=preview&styleid=$redirect_id");
      wp_register_script('ano-6310-redirect-script', '');
      wp_enqueue_script('ano-6310-redirect-script');
      wp_add_inline_script('ano-6310-redirect-script', "document.location.href = '" . $url . "';");
    }
  }
}
$cssData = [];
